...
public class EmergenciaController {
    ...
    private Set<Integer> emergenciasMostradas = new HashSet<>(); // Conjunto para almacenar IDs de emergencias ya mostradas
    private Set<String> notificacionesVistas = new HashSet<>(); // Conjunto para almacenar descripciones únicas de notificaciones
            ...
            public void run() {
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        String claveNotificacion = descripcion + gravedad + tipo;

                        // Verificar si la notificación ya ha sido mostrada
                        if (!notificacionesVistas.contains(claveNotificacion) && !emergenciasMostradas.contains(idEmergencia)) {
                            // Agregar la clave al conjunto de notificaciones vistas
                            notificacionesVistas.add(claveNotificacion);
                            ...
                            nuevaVentana.initfrmNotificacionEmergencia(descripcion, gravedad, tipo, idEmergencia); // Pasa el ID aquí
